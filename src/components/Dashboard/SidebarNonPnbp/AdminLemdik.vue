...
onMounted(() => {
  if (storeApp.tokenAdminLemdik) {
    refreshToken();
  }
  user.value = JSON.parse(storeApp.userAdminLemdik);
});
  ...
  axios
    ...
    .catch((err) => {
      ...
      confirmation(
        ...
        localStorage.removeItem("tokenAdminLemdik");
        localStorage.removeItem("userAdminLemdik");
        window.close();
      });
    });
}
