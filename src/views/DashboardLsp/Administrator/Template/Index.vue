...
onMounted(() => {
  fetchData(`${storeApp.baseurl}cbt/lsp/admin-pusbang/template`);
});
...
function destroy(data) {
  confirmation("Yakin hapus data ini?").then((confirmed) => {
    if (confirmed) {
      ...
      axios
        .delete(
          `${storeApp.baseurl}cbt/lsp/admin-pusbang/template/${data.id}`,
          {
            headers: {
              Authorization: `Bearer ${storeApp.tokenAdministratorPelatihan}`,
            },
      ...
      <div class="container p-lg-4">
        <div class="d-flex px-2 mb-4 justify-content-between">
          ...
          <div class="col-5 col-lg-3 text-end mb-3">
            <router-link
              :to="{ name: 'dashboard-lsp-administrator-template-create' }"
