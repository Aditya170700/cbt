...
onMounted(() => {
  form.loadingData = true;
  axios
    .get(
      `${storeApp.baseurl}cbt/lsp/admin-pusbang/template/${route.params.id}/show`,
      {
        headers: {
    ...
    .then((res) => {
      ...
      form.loadingData = false;
      form.name = res.data.data.name;
      res.data?.data?.pertanyaan?.forEach((val) => {
        form.selected.push(val.pertanyaan);
        params.selected_ids.push(val.pertanyaan.id);
      });

      fetchSoal(`${storeApp.baseurl}cbt/lsp/admin-pusbang/soal/for-template`);
    })
    .catch((err) => {
      form.loadingData = false;
      console.log(err);
...
function submit() {
  ...
  axios
    .put(
      `${storeApp.baseurl}cbt/lsp/admin-pusbang/template/${route.params.id}`,
      {
        name: form.name,
    ...
    .then((res) => {
      ...
      router.push({ name: "dashboard-lsp-administrator-template" });
    })
    .catch((err) => {
      form.loading = false;
      ...
      <div class="container p-lg-4">
        <div class="d-flex px-2 mb-4 justify-content-between">
          <div class="col-7 col-lg-9 text-start mb-3">
            <div class="h4 fw-bold">EDIT TEMPLATE SOAL</div>
          </div>
          <div class="col-5 col-lg-3 text-end mb-3">
            <router-link
              :to="{ name: 'dashboard-lsp-administrator-template' }"
        ...
        <div class="row px-2" v-if="form.loadingData">
                  ...
                  <div class="col-6">
                    <div class="input-group mb-3 bg-white rounded-2 shadow">
                      ...
                      <button
                        ...
                        @click="
                          fetchSoal(
                            `${storeApp.baseurl}cbt/lsp/admin-pusbang/soal/for-template`
                          )
                        "
                      >
