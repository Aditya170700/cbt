...
onMounted(() => {
  ...
  axios
    .get(
      `${storeApp.baseurl}cbt/lsp/admin-pusbang/soal/${route.params.id_soal}/show`,
      {
        headers: {
...
function submit() {
  ...
  axios
    .put(
      `${storeApp.baseurl}cbt/lsp/admin-pusbang/soal/${route.params.id_soal}/update`,
      form,
      {
        headers: {
          Authorization: `Bearer ${storeApp.tokenAdministratorPelatihan}`,
    ...
    .then((res) => {
      ...
      router.push({ name: "dashboard-lsp-administrator-soal" });
    })
    .catch((err) => {
      form.submitLoading = false;
...
function destroyOpsi(data, iopt) {
  confirmation("Yakin hapus data ini?").then((confirmed) => {
    if (confirmed) {
      ...
      axios
        .delete(
          `${storeApp.baseurl}cbt/lsp/admin-pusbang/soal/${route.params.id_soal}/destroy-opsi/${data.id}`,
          {
            headers: {
              Authorization: `Bearer ${storeApp.tokenAdministratorPelatihan}`,
            },
      ...
      <div class="container p-lg-4">
        <div class="d-flex px-2 mb-3 justify-content-between">
          ...
          <div class="col-6 text-end">
            <router-link
              :to="{ name: 'dashboard-lsp-administrator-soal' }"
