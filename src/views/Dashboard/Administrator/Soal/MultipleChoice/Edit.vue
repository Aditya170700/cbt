...
onMounted(() => {
  ...
  axios
    .get(
      `${storeApp.baseurl}cbt/admin-pusbang/soal/${route.params.id_soal}/show`,
      {
        headers: {
...
function submit() {
  ...
  axios
    .put(
      `${storeApp.baseurl}cbt/admin-pusbang/soal/${route.params.id_soal}/update`,
      form,
      {
        headers: {
          Authorization: `Bearer ${storeApp.tokenAdministratorPelatihan}`,
    ...
    .then((res) => {
      ...
      router.push({ name: "dashboard-administrator-soal" });
    })
    .catch((err) => {
      form.submitLoading = false;
...
function destroyOpsi(data, iopt) {
  confirmation("Yakin hapus data ini?").then((confirmed) => {
    if (confirmed) {
      ...
      axios
        .delete(
          `${storeApp.baseurl}cbt/admin-pusbang/soal/${route.params.id_soal}/destroy-opsi/${data.id}`,
          {
            headers: {
              Authorization: `Bearer ${storeApp.tokenAdministratorPelatihan}`,
            },
