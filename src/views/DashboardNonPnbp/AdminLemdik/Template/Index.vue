...
onMounted(() => {
  fetchData(`${storeApp.baseurl}cbt/non-pnbp/admin-lemdik/template`);
});
...
function destroy(data) {
  confirmation("Yakin hapus data ini?").then((confirmed) => {
    if (confirmed) {
      ...
      axios
        .delete(
          `${storeApp.baseurl}cbt/non-pnbp/admin-lemdik/template/${data.id}`,
          {
            headers: {
              Authorization: `Bearer ${storeApp.tokenAdminLemdik}`,
            },
          ...
          <div class="col-5 col-lg-3 text-end mb-3">
            <div class="input-group mb-3 bg-white rounded-2 shadow">
              ...
              <button
                ...
                @click="
                  fetchData(
                    `${storeApp.baseurl}cbt/non-pnbp/admin-lemdik/template`
                  )
                "
              >
