...
onMounted(() => {
  ...
  axios
    .get(
      `${storeApp.baseurl}cbt/non-pnbp/admin-lemdik/template/${route.params.id}/show`,
      {
        headers: {
    ...
    .then((res) => {
      ...
      fetchSoal(
        `${storeApp.baseurl}cbt/non-pnbp/admin-lemdik/soal/for-template`
      );
    })
    .catch((err) => {
      form.loadingData = false;
...
function submit() {
  ...
  axios
    .put(
      `${storeApp.baseurl}cbt/non-pnbp/admin-lemdik/template/${route.params.id}`,
      {
        name: form.name,
